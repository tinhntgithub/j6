...
app.controller("checkoutCtrl", function($scope, $http) {
	...
	let message = "";
	let status = true;
	let orderStatus = window.location.search.substr(1);
	...
	const createOrder = function(mess) {
		...
		$http.post(url, data).then(resp => {
			alertSuccess(mess)
			setTimeout(function() { location.href = "/manageOrders.html" }, 1000);

		}).catch(error => {
			alertDanger("Thanh toán thât bại")
			console.log(error);
		});
	...
	if (orderStatus == "err=bug") {
		message = "Thanh toán thất bại";
	} else if (orderStatus == "mess=orderCancel") {
		message = "Đã hủy thanh toán VNPay";
	} 
	else if (orderStatus == "mess=orderSuccess") {
		$scope.$watch("payFromCart", function(value) {//I change here
			var val = value || null;
			console.log(val)
			if (val.length > 0) {
				createOrder("Thanh toán VNPay thành công");
			}else{
